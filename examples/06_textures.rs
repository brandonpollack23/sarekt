    ...
    *control_flow = ControlFlow::Poll;
    ...
    match event {
      Event::MainEventsCleared => {
        ...
        let max_camera_height = 1.5f32;
        camera_height = (camera_rate * time_since_start_secs) % (2.0f32 * max_camera_height);
        if camera_height >= max_camera_height {
          camera_height = (2.0f32 * max_camera_height) - camera_height;
        }
...
fn update_uniforms(
  ...
  let model_matrix =
    uv::Mat4::from_translation(spoderman_position) * uv::Mat4::from_rotation_y(spoderman_rotation);
  let view_matrix = uv::Mat4::look_at(
    /* eye= */ uv::Vec3::new(0.0f32, camera_height, 0.0f32),
    /* at= */ spoderman_position,
    /* up= */ uv::Vec3::unit_y(),
  );
